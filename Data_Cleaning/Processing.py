...
CENTROID_CACHE_PATH = BASE_DIR / "parquet_centroids_cache.parquet"

def get_parquet_with_centroids(parquet_df):
    if CENTROID_CACHE_PATH.exists():
        print("Loading cached Parquet centroids...")
        return pd.read_parquet(CENTROID_CACHE_PATH)

    print("Computing Parquet centroids (one-time, will cache)...")
    lats, lngs = [], []
    for geom in parquet_df["geometry"]:
        if pd.isna(geom):
            lats.append(None); lngs.append(None); continue
        try:
            poly = wkt.loads(geom) if isinstance(geom, str) else wkb.loads(geom)
            lats.append(poly.centroid.y); lngs.append(poly.centroid.x)
        except Exception:
            lats.append(None); lngs.append(None)
    parquet_df = parquet_df.copy()
    parquet_df["centroid_lat"] = lats
    parquet_df["centroid_lng"] = lngs
    parquet_df.to_parquet(CENTROID_CACHE_PATH)
    return parquet_df
...
def mesh_all_years(xlsx_df, parquet_df, output_path, start_year=None, end_year=None):
    print("Cross-referencing and meshing all years...")

    xlsx_us = xlsx_df[xlsx_df["Country"].str.contains("United States|USA", case=False, na=False)].copy()

    years = sorted(set(xlsx_us["Year"].dropna().astype(int)) | set(parquet_df["Year"].dropna().astype(int)))
    if start_year: years = [y for y in years if y >= start_year]
    if end_year:   years = [y for y in years if y <= end_year]

    meshed_points = []
    totals = {"scanned": 0, "no_geom": 0, "not_us": 0, "matched": 0, "parquet_only": 0}

    for year in years:
        csv_year = xlsx_us[xlsx_us["Year"] == year]
        parquet_year = parquet_df[parquet_df["Year"] == year]
        matched_xlsx_ids = set()

        for p_idx, p_row in parquet_year.iterrows():
            totals["scanned"] += 1
            geom = p_row["geometry"]
            if pd.isna(geom):
                totals["no_geom"] += 1
            ...
            p_end = pd.to_datetime(p_row["end_date"])
            time_match = csv_year[
                (pd.to_datetime(csv_year["Start Date"]) <= p_end + pd.Timedelta(days=4)) &
                (pd.to_datetime(csv_year["End Date"]) >= p_start - pd.Timedelta(days=4))
            ]

            if time_match.empty:
                totals["parquet_only"] += 1
                meshed_points.append({
                    "id": str(p_row.get("uuid", p_idx)), "year": int(year),
                    "lat": float(lat), "lng": float(lng),
                    "source_flags": {"in_parquet": True, "in_csv": False, "warnings": []},
                    "metrics": {
                        "parquet_area_km2": float(p_row["area_km2"]),
                        "parquet_duration_days": int(p_row["Duration (days)"]),
                        "start_date": str(p_start.date()),   # ← add this line to both branches
                        "csv_area_km2": None, "csv_duration_days": None,
                        "fatalities": None, "displaced": None, "matched_event_id": None
                    }})
                continue

            match_row = time_match.iloc[0]
            matched_xlsx_ids.add(match_row.name)
            totals["matched"] += 1
            meshed_points.append({
                "id": str(p_row.get("uuid", p_idx)), "year": int(year),
                "lat": float(lat), "lng": float(lng),
                "source_flags": {"in_parquet": True, "in_csv": True, "warnings": []},
                "metrics": {"parquet_area_km2": float(p_row["area_km2"]),
                            "parquet_duration_days": int(p_row["Duration (days)"]),
                            "csv_area_km2": float(match_row["area_km2"]),
                            "csv_duration_days": int(match_row["Duration (days)"]),
                            "fatalities": int(match_row.get("Fatalities", 0)),
                            "displaced": int(match_row.get("Displaced", 0)),
                            "matched_event_id": str(match_row.name)}
            })

        for c_idx, c_row in csv_year.iterrows():
            if c_idx in matched_xlsx_ids:
                continue
            meshed_points.append({
                "id": f"xlsx-{c_idx}", "year": int(year),
                "lat": float(c_row["Latitude"]), "lng": float(c_row["Longitude"]),
                "source_flags": {"in_parquet": False, "in_csv": True, "warnings": []},
                "metrics": {"parquet_area_km2": None, "parquet_duration_days": None,
                            "csv_area_km2": float(c_row["area_km2"]),
                            "csv_duration_days": int(c_row["Duration (days)"]),
                            "fatalities": int(c_row.get("Fatalities", 0)),
                            "displaced": int(c_row.get("Displaced", 0)),
                            "matched_event_id": None}
            })

        print(f"  {year}: matched so far {totals['matched']:,} | parquet-only {totals['parquet_only']:,}")

    print(f"\n  TOTAL: {len(meshed_points):,} points | matched(purple) {totals['matched']:,} | "
          f"parquet-only(red) {totals['parquet_only']:,} | not-US skipped {totals['not_us']:,}")

    with open(output_path, "w") as f:
        json.dump(meshed_points, f, indent=2)
    print(f"✓ Wrote {len(meshed_points)} points to {output_path}")
    return meshed_points
...
def main():
    ...
    if xlsx_df is not None and parquet_df is not None:
        ...
        parquet_df = get_parquet_with_centroids(parquet_df)
        UNIFIED_OUTPUT = REACT_DATA_DIR / "points_all_years.json"
        mesh_all_years(xlsx_df, parquet_df, UNIFIED_OUTPUT)
